...
func describeCLIWithGoModulesEnabled(useGoModules bool) interface{} {
	mode := "with GOPATH"
	if useGoModules {
		mode = "with Go modules"
	}

	Describe("CLI "+mode, func() {
...
`+packageDir+`/matchers
`+packageDir+`/matchers/ptr_to_http_request.go
`+packageDir+`/mock_mydisplay_test.go
`+packageDir+`/mock_requesthandler_test.go
`+packageDir+`/subpackage/matchers
`+packageDir+`/subpackage/matchers/ptr_to_http_request.go
`+packageDir+`/subpackage/mock_requesthandler.go
`+packageDir+`/subpackage/mock_subdisplay.go
`), buf.String())
								Expect(joinPath(packageDir, "mock_mydisplay_test.go")).NotTo(BeAnExistingFile())
								Expect(joinPath(subPackageDir, "mock_subdisplay.go")).NotTo(BeAnExistingFile())
