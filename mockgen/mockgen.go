...
func GenerateOutput(ast *model.Package, source, packageOut, selfPackage, vendorPath string) ([]byte, error) {
	g := new(generator)
	g.generateCode(source, ast, packageOut, selfPackage, vendorPath)
...
func (g *generator) generateCode(source string, pkg *model.Package, pkgName, selfPackage, vendorPath string) {
	...
	packageMap, nonVendorPackageMap := generateUniquePackageNamesFor(importPaths, vendorPath)
...
func generateUniquePackageNamesFor(importPaths map[string]bool, vendorPath string) (packageMap, nonVendorPackageMap map[string]string) {
	...
	for importPath := range importPaths {
		...
		if strings.HasPrefix(importPath, vendorPath+"/") {
			vendorParsedImportPath = strings.Replace(importPath, vendorPath+"/", "", 1)
		}
